...
namespace CodeCake
{
    public class YarnTempFileTextModification : IDisposable
    {
        ...
        protected YarnTempFileTextModification( YarnTempFileTextModification toCopy )
        {
            _originalText = toCopy._originalText;
            _path = toCopy._path;
        }

        public static (string content, YarnTempFileTextModification temp) CreateTempFileTextModification( NormalizedPath path )
        {
            bool fileExist = File.Exists( path );
            if( !fileExist ) File.Create( path ).Dispose();
            string txt = File.ReadAllText( path );
            return (txt, new YarnTempFileTextModification( txt, path, fileExist ));
        }
        ...
        public static IDisposable TemporaryReplacePackageVersion(
            ...
            SVersion version)
        {
            (string content, YarnTempFileTextModification temp) = CreateTempFileTextModification( jsonPath );
                ...
                if( json.ContainsKey( dependencyPropName ) )
                {
                    ...
                    foreach( KeyValuePair<string, JToken> keyValuePair in dependencies )
                    {
                        if( npmSolution.AllPublishedProjects.FirstOrDefault( x => x.PackageJson.Name == keyValuePair.Key )
                            is YarnPublishedProject localProject )
                        {
        ...
        public static YarnTempFileTextModification TemporaryInjectNPMToken( NormalizedPath npmrcPath, string pushUri, string scope, Action<List<string>, string> configure )
        {
            (string content, YarnTempFileTextModification temp) = CreateTempFileTextModification( npmrcPath );
