...
namespace CodeCake
{
    public class YarnPublishedProject : YarnProject, ILocalArtifact
    {
        readonly bool _ckliLocalFeedMode;

        YarnPublishedProject( StandardGlobalInfo globalInfo, YarnSolution npmSolution, YarnSimplePackageJsonFile json, NormalizedPath outputPath )
        ...
        /// <summary>
        /// Create a <see cref="YarnProject"/> that can be a <see cref="YarnPublishedProject"/>.
        /// </summary>
        ...
        /// <returns></returns>
        public static YarnProject Create( StandardGlobalInfo globalInfo, YarnSolution solution, NormalizedPath dirPath, NormalizedPath outputPath )
        {
            var json = YarnSimplePackageJsonFile.Create( globalInfo.Cake, dirPath );
            YarnProject output;
            if( json.IsPrivate )
            {
                output = CreateYarnProject( globalInfo, solution, json, outputPath );
            }
            else
            {
                output = new YarnPublishedProject( globalInfo, solution, json, outputPath );
            }
            return output;
        }
